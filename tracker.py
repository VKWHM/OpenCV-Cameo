...
class FaceTracker(object):
	def __init__(self, scaleFactor = 1.3, minNeighbors = 2, flags = cv2.CASCADE_SCALE_IMAGE, logger = 'FaceTracker'):
	...
	def update(self, image):
		...
		facesRect = self.faceClassifier.detectMultiScale(image, self.scaleFactor, self.minNeighbors, self.flags, minSize)
		if facesRect is not None:
			for faceRect in facesRect:
				...
				searchRect = (x+w*3//4, y, w//4, h)
				lEyeRect = self._detectOneObject(self.eyeClassifier, image, searchRect, 64)
				if lEyeRect:
					self._logger.debug(f"Left eye detected")
				face.leftEyeRect = lEyeRect
				searchRect = (x, y, w*3//4, h)
				rEyeRect = self._detectOneObject(self.eyeClassifier, image, searchRect, 64)
				if rEyeRect:
	...
	def _detectOneObject(self, classifier, image, searchRect, imageSizeToMinSizeRatio):
		...
		objectRect = classifier.detectMultiScale(image, self.scaleFactor, self.minNeighbors, self.flags, minSize)
		if len(objectRect) == 0:
			return
	...
	def drawDebugRects(self, image):
		...
		if utils.isGray(image):
			...
			colors = {'faceRect': (255, 255, 255),'leftEyeRect': (0, 0, 255),'rightEyeRect': (0, 255, 255)}
        
		for face in self._faces:
			for attribute in list(filter(check, [x for x in dir(face) if not x.startswith('_')])): 
				utils.outlineRect(image, getattr(face, attribute), colors.get(attribute, 255))
